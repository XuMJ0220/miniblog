...
func New(opts *Options) *zapLogger {
	...
	if err := zapLevel.UnmarshalText([]byte(opts.Level)); err != nil {
		// 如果制定了非法的日志级别, 则默认使用 info 级别
		zapLevel = zapcore.InfoLevel
	}

	// 创建 encoder 配置, 用于控制日志的输出格式
	encoderConfig := zap.NewProductionEncoderConfig()
	// 自定义 MessageKey 为 message, message 语义更准确
	encoderConfig.MessageKey = "message"
	// 自定义 TimeKey 为 timestamp, timestamp 语义更准确
	encoderConfig.TimeKey = "timestamp"
	// 指定时间序列化函数, 将时间序列化为 `2006-01-02 15:04:05.000` 格式, 更易读
	encoderConfig.EncodeTime = func(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
		enc.AppendString(t.Format("2006-01-02 15:04:05.000"))
	}
	// 指定 time.Duration 序列化函数, 将 time.Duration 序列化为经过毫秒数的浮点数
	// 毫秒数比默认的秒数更精准
