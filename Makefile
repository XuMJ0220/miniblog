...
## 指定应用使用的 version 包, 会通过 `-ldflags -X` 向该包中指定的便来嗯注入值
...
GO_LDFLAGS += \
	...
	-X $(VERSION_PACKAGE).GIT_TREE_STATE=$(GIT_TREE_STATE) \
...
# 定义 Makefile all 为目标，执行 ’make‘ 时，默认执行 all 为目标
.PHONY: all
...
ca: # 生成 CA 文件.
	...
	@# 1. 生成根证书私钥 (CA Key)
	@openssl genrsa -out $(OUTPUT_DIR)/cert/ca.key 2048
	...
	@openssl req -new -nodes -key $(OUTPUT_DIR)/cert/ca.key -sha256 -days 3650 -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "/C=CN/ST=Guangdong/L=Shenzhen/O=xumj/OU=it/CN=127.0.0.1/emailAddress=1044011439@qq.com"
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "/C=CN/ST=Guangdong/L=Shenzhen/O=serverdevops/OU=serverit/CN=localhost/emailAddress=1044011439@qq.com" \
